...
{
//	cout << "rrip miss" <<m_pstate[set] << endl;
/*	switch(m_pstate[set]) {
		case NONE:
			break;
		case FOLLOWER:
...
/*			
	...
	switch(m_pstate[set_id])
	{
		case NONE:
		case FOLLOWER:
//			cout << "cache_line_set:" << set_id <<endl;
...
*/	//protected distance
	pd=8;
	ins_line->m_last_access_time=DISTANT_RRPV-pd;
	ins_line->isprotected=1;
...
cache_entry_c* rrip_cache_c::find_replacement_line(int set, int appl_id)
{
	...
	while(1) {
		...
		for(int ii = 0; ii < m_assoc; ii++)
		{
			...
			if(line->isprotected==1){
				pdcount++;
			}


			else if((line->m_last_access_time >= DISTANT_RRPV)&&(line->isprotected==0))
			{
		...
		if (pdcount==m_assoc)//every way is protected then erase one
			pdmaxflag=1;
		if (RRIP_tail_ind == -1) // Can't find long RRPV, do aging for all entry
		{
			for (int ii = 0; ii < m_assoc; ii++)
			{
				...
				if(first==0){
					...
					if(line->isprotected==0)
						line->m_last_access_time++;
						first++;
				}
				else first++;

				if(pdmaxflag==1&&line->m_last_access_time>=20){
					line->isprotected=0;
					pdmaxflag=0;
				}
